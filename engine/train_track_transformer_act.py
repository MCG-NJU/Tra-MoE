...
# from atm.model import TrackACT
# from atm.dataloader import ATM3DPretrainDiffusionForcingDataset, ATMGTPretrainDataset, get_dataloader
# from atm.utils.log_utils import BestAvgLoss, MetricLogger
# from atm.utils.train_utils import init_wandb, setup_lr_scheduler, setup_optimizer
...
def main(cfg: DictConfig):
    ...
    train_dataset = ATMPretrainDataset(
        dataset_dir=cfg.train_dataset, **cfg.dataset_cfg, aug_prob=cfg.aug_prob
    )
    ...
    train_vis_dataset = ATMPretrainDataset(
        dataset_dir=cfg.train_dataset,
        vis=True,
        **cfg.dataset_cfg,
        aug_prob=cfg.aug_prob,
    )
    train_vis_dataloader = get_dataloader(
        train_vis_dataset, mode="train", num_workers=1, batch_size=1
    )

    val_dataset = ATMPretrainDataset(
        dataset_dir=cfg.val_dataset, **cfg.dataset_cfg, aug_prob=0.0
    )
    ...
    val_vis_dataset = ATMPretrainDataset(
        dataset_dir=cfg.val_dataset, vis=True, **cfg.dataset_cfg, aug_prob=0.0
    )
    val_vis_dataloader = get_dataloader(
        val_vis_dataset, mode="val", num_workers=1, batch_size=1
    )
        ...
        if fabric.is_global_zero:
            ...
            if epoch % cfg.save_freq == 0:
                ...
                def vis_and_log(model, vis_dataloader, mode="train"):
                    vis_dict = visualize(
                        model, vis_dataloader, mix_precision=cfg.mix_precision
                    )
                    ...
                    (
                        ...
                        if cfg.dry
                            ...
                            {f"{mode}/reconstruct_track": wandb_vis_track}, step=epoch
                        )
                    )
...
def run_one_epoch(
    ...
    for vid, track, vis, task_emb in tqdm(dataloader):
        ...
        batch = {"vid": vid,"track": track,"task_emb": task_emb}
...
@torch.no_grad()
def evaluate(
    model, dataloader, lbd_track, lbd_img, p_img, mix_precision=False, tag="val"
):
    track_loss, vid_loss, tot_loss, tot_items = 0, 0, 0, 0
    model.eval()
    ...
    for vid, track, vis, task_emb in tqdm(dataloader):
        ...
        batch = {"vid": vid, "track": track,"task_emb": task_emb, "vis": vis}
