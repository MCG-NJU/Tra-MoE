...
def rollout(env_dict, policy, num_env_rollouts, horizon=None, return_wandb_video=True,
    ...
    for env_description, (env_idx, env) in env_dict.items():
        ...
        for _ in tqdm(range(num_env_rollouts)):
            ...
            last_state = {k: obs[obs_key_mapping[k]] for k in policy.extra_state_keys}
            ...
            while not done and (horizon is None or step_i < horizon):
                ...
                if "depths" in obs:
                    extra_states["depths"] = obs["depths"]
                if "intrinsics" in obs:
                    extra_states["intrinsics"] = obs["intrinsics"]

                a, _tracks = policy("act", rgb, task_emb, extra_states)

                # a, _tracks = policy("act", rgb, task_emb, extra_states, last_frame=last_frame)
                # last_frame = rgb
                # last_state = extra_states.copy()
