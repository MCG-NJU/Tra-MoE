...
class TrackTransformer(nn.Module):
    ...
    def _init_track_modules(self, dim, num_track_ts, num_track_ids, patch_size=1, track_dim=2):
        ...
        self.track_decoder = nn.Linear(dim, track_dim * patch_size, bias=True)
        self.num_track_ids = num_track_ids
        self.num_track_ts = num_track_ts
    ...
    def forward_(self, vid, track, task_emb, p_img):
        """
        ...
        return rec_track, rec_patches, los
    ...
    def reconstruct(self, vid, track, task_emb, p_img):
        ...
        aaa, bbb, _ = self.forward_(vid, track, task_emb, p_img)
        #aaa = self.forward_(vid, track, task_emb, p_img)
    ...
    def forward(self, vis_or_loss, *args, **kwargs):
        if vis_or_loss == "vis":
            return self.forward_vis(*args, **kwargs)
        elif vis_or_loss == "loss":
            #kkk = self.forward_vis(*args, **kwargs)
    ...
    def forward_loss(self,
        ...
        rec_track, rec_patches, los = self.forward_(vid, track, task_emb, p_img)
        #rec_track, rec_patches = self.forward_(vid, track, task_emb, p_img)

        vis[vis == 0] = .1
        vis = repeat(vis, "b tl n -> b tl n c", c=self.track_dim)

        track_loss = torch.mean((rec_track - track) ** 2 * vis)
        img_loss = torch.mean((rec_patches - self._patchify(vid)) ** 2)
        loss = lbd_track * track_loss + lbd_img * img_loss + los
    ...
    def project_3d_tracks_to_2d(self, track, intrinsics):
        ...
        track_flat = track.view(b, -1, 3)  # shape: (b, N, 3), where N is the product of the remaining dimensions

        # Apply the intrinsic matrix to the 3D points
        projected_homogeneous = torch.bmm(track_flat, intrinsics.transpose(1, 2))  # shape: (b, N, 3)

        # Normalize by the depth (Z-coordinate)
        projected_points = projected_homogeneous[..., :2] / projected_homogeneous[..., 2:3]  # shape: (b, N, 2)

        # Reshape back to the original batch dimensions
        projected_points = projected_points.view(*track.shape[:-1], 2)  # shape: (b, ..., 2)

        return projected_points
        
    
    def forward_vis(self, vid, track, task_emb, p_img, **kwargs):
        """
        track: (b, tl, n, 2)
        vid: (b, t, c, h, w)
        """
        ...
        rec_track, rec_patches, _ = self.forward_(vid, track, task_emb, p_img)
        ...
        if self.track_dim == 3 and "intrinsics" in kwargs:
            intrinsics = kwargs["intrinsics"]   # b, 3, 3
            rec_track = self.project_3d_tracks_to_2d(rec_track, intrinsics)
            rec_track[..., 0] = rec_track[..., 0] / H
            rec_track[..., 1] = rec_track[..., 1] / W
            # breakpoint()
        track = track.clone()
