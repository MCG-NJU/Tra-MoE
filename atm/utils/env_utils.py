...
class LiberoImageUpsideDownWrapper(Wrapper):
    ...
    def reset(self):
        ...
        if "agentview_depth" in obs:
            obs["agentview_depth"] = obs["agentview_depth"][:, ::-1, ...]
        if "robot0_eye_in_hand_depth" in obs:
            obs["robot0_eye_in_hand_depth"] = obs["robot0_eye_in_hand_depth"][:, ::-1, ...]
        return obs
...
class Libero3DObservationWrapper(ObservationWrapper):
    valid_obs_types = ["image", "depth"]
    ...
    def _stack_obs(self, obs, axis=0):
        ...
        for t in self.valid_obs_types:
            obs_dict[t] = []
            for c in self.cameras:
                assert f"{c}_{t}" in obs, f"{c}_{t} not in obs, valid obs types: {obs.keys()}"
                mod = obs[f"{c}_{t}"]
                obs_dict[t].append(mod)
            obs_dict[t] = np.stack(obs_dict[t], axis=1)  # (b, v, h, w, c)
            if t == "depth":
                obs_dict[t] = obs_dict[t].squeeze(-1)   # (b, v, h, w)
        obs_dict["depths"] = obs_dict.pop("depth")
        return obs_dict
...
class LiberoIntrinsicsWrapper(Wrapper):
    def __init__(self, env, camera_height, camera_width):
        super(LiberoIntrinsicsWrapper, self).__init__(env)
    ...
    def _get_intrinsics(self):
        ...
        for cam_name in self.cameras:
            cam_id = self.env.env.sim.model.camera_name2id(cam_name)
            fovy = self.env.sim.model.cam_fovy[cam_id]
            f = 0.5 * self.camera_height / np.tan(fovy * np.pi / 360)
            K = np.array([[f, 0, self.camera_width / 2], [0, f, self.camera_height / 2], [0, 0, 1]])
            intrinsics.append(K)
        return np.stack(intrinsics, axis=0) # v, 3, 3
        
    def reset(self):
        obs = self.env.reset()
        obs["intrinsics"] = self._get_intrinsics()
        return obs
    ...
    if env_type.lower() == "libero":
        ...
        for env_idx in range(idx_start, idx_end):
            ...
            env, task_embs = make_libero_env(e_name, t_name, img_h, img_w, task_embedding=task_embedding,
                                             gpu_id=gpu_id, vec_env_num=vec_env_num, seed=seed, cameras=cameras, **kwargs)
            ...
            if kwargs.get("camera_depths", False):
                env = Libero3DObservationWrapper(env, masks=None, cameras=cameras)
                # env = LiberoIntrinsicsWrapper(env, img_h, img_w)
            else:
