...
from atm.dataloader.utils import (
    load_rgb,
    ImgTrackColorJitter,
    ImgViewDiffTranslationAug,
)
...
class BaseDataset(Dataset):
    def __init__(
        ...
        if not cache_all:
            assert (
                not cache_image
            ), "cache_image is only supported when cache_all is True."
        ...
        for dir_idx, d in enumerate(self.dataset_dir):
            if "libero_90" in d:
                d = d.replace("/train", "")
                fn_list = [os.path.join(d, f"demo_{i}.hdf5") for i in range(20)]
            
            elif "atm_rlbench" in d:
                d = d.replace("/train", "")
                fn_list = [os.path.join(d, f"demo_{i}.hdf5") for i in range(5)]
            else:
            ...
            if self.num_demos is None:
                ...
                assert (
                    0 < self.num_demos <= 1
                ), "num_demos means the ratio of training data among all the demos."
                n_demo = int(len(fn_list) * self.num_demos)
            for fn in fn_list[:n_demo]:
                self.buffer_fns.append(fn)
        assert len(self.buffer_fns) > 0
        print(
            f"found {len(self.buffer_fns)} trajectories in the specified folders: {self.dataset_dir}"
        )
        ...
        self.augmentor = transforms.Compose(
            [
                ...
                ImgViewDiffTranslationAug(
                    ...
                    translation=8,
                    augment_track=self.augment_track,
                ),
            ]
        )
    ...
    def load_demo_info(self):
        ...
        for demo_idx, fn in enumerate(self.buffer_fns):
            ...
            if self.cache_all:
                ...
                self._cache.append((demo_idx, demo))
            self._demo_id_to_path[demo_idx] = fn
            self._index_to_demo_id.update(
                {k: demo_idx for k in range(start_idx, start_idx + demo_len)}
            )
    ...
    def process_demo(self, demo):
        ...
        for v in self.views:
            ...
            vids = np.concatenate(
                [vids, np.repeat(last_frame, pad_length, axis=0)], axis=0
            )
            tracks = np.concatenate(
                [tracks, np.repeat(last_track, pad_length, axis=0)], axis=0
            )
            ...
            if h != self.img_size[0] or w != self.img_size[1]:
                vids = F.interpolate(
                    vids, size=self.img_size, mode="bilinear", align_corners=False
                )
        ...
        if "extra_states" in demo["root"]:
            extra_states_dict = {
                k: demo["root"]["extra_states"][k] for k in self.extra_state_keys
            }
        ...
        if "actions" in demo["root"]:
            ...
            actions = np.concatenate(
                [actions, np.repeat(zero_action, pad_length, axis=0)], axis=0
            )
        ...
        if "extra_states" in demo["root"]:
            ...
            extra_states_dict = {
                k: np.concatenate(
                    [
                        extra_states_dict[k],
                        np.repeat(last_extra_states[k], pad_length, axis=0),
                    ],
                    axis=0,
                )
    ...
    def _load_image_list_from_demo(
        self, demo, view, time_offset, num_frames=None, backward=False
    ):
        ...
        if backward:
            image_indices = np.arange(
                max(time_offset + 1 - num_frames, 0), time_offset + 1
            )
            ...
            if len(frames) < num_frames:
                padding_frames = torch.zeros(
                    (num_frames - len(frames), *frames.shape[1:])
    ...
    def _load_image_list_from_disk(
        self, demo_id, view, time_offset, num_frames=None, backward=False
    ):
        num_frames = self.frame_stack if num_frames is None else num_frames
        ...
        if "libero_90" in demo_path:
            demo_parent_dir = os.path.dirname(demo_path)
        
        elif "atm_rlbench" in demo_path:
            demo_parent_dir = os.path.dirname(demo_path)
        ...
        if "rlbench" in images_dir:
                if view == "agentview":
                    view = "left_shoulder_rgb"
                if view == "eye_in_hand":
                    view = "wrist_rgb"
        ...
        if backward:
            ...
            frames = [
                self.load_image_func(os.path.join(images_dir, f"{view}_{img_idx}.png"))
                for img_idx in image_indices
            ]
            frames = [
                np.zeros_like(frames[0]) for _ in range(num_frames - len(frames))
            ] + frames  # padding with black images
        else:
