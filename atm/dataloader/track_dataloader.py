...
class ATMPretrainDataset(BaseDataset):
    ...
    def load_demo_info(self):
        ...
        for demo_idx, fn in enumerate(self.buffer_fns):
            ...
            if 'left_shoulder_rgb' in demo['root']:
               
                demo['root']['agentview'] = demo['root'].pop('left_shoulder_rgb')
                
            if 'wrist_rgb' in demo['root']:
                
                demo['root']['eye_in_hand'] = demo['root'].pop('wrist_rgb')
            ...
            if self.views is None:
                ...
                if "actions" in self.views:
                    self.views.remove("actions")

                if "task_emb_bert" in self.views:
                    self.views.remove("task_emb_bert")

                if "extra_states" in self.views:
            ...
            self._index_to_demo_id.update(
                {k: demo_idx for k in range(start_idx, start_idx + demo_len * 2)}
            )
            self._index_to_view_id.update(
                {
                    k: (k - start_idx) % 2
                    for k in range(start_idx, start_idx + demo_len * 2)
                }
            )
    ...
    def __getitem__(self, index):
        ...
        if self.cache_all:
            ...
            if self.cache_image:
                vids = self._load_image_list_from_demo(
                    demo, view, time_offset, backward=True
                )  # t c h w
            else:
                vids = self._load_image_list_from_disk(
                    demo_id, view, time_offset, backward=True
                )  # t c h w
        else:
        ...
        tracks = demo["root"][view]["tracks"][
            time_offset : time_offset + self.num_track_ts
        ]  # track_len n 2
        vis = demo["root"][view]["vis"][
            time_offset : time_offset + self.num_track_ts
        ]  # track_len n
        ...
        if np.random.rand() < self.aug_prob:
            vids = vids[
                None
            ]  # expand to (1, t, c, h, w) to fit the input shape of random shift augmentation
            tracks = tracks[
                None, None
            ]  # expand to (1, 1, track_len, n, 2) to fit the input shape of random shift augmentation
            vids, tracks = self.augmentor((vids / 255.0, tracks))
            vids = vids[0, ...] * 255.0
            tracks = tracks[0, 0, ...]

        # sample tracks
        tracks, vis = sample_tracks_visible_first(
            tracks, vis, num_samples=self.num_track_ids
        )
