...
class LiberoIntrinsicsWrapper(Wrapper):
    """ Wrap the complex state initialization process in LIBERO """
    def __init__(self, env, camera_height, camera_width, cameras):
        super().__init__(env)
...
def make_libero_env(task_suite_name, task_name, img_h, img_w, task_embedding=None, gpu_id=-1, vec_env_num=1, seed=0, **kwargs):
    """
    ...
    if task_embedding is None:
        ...
        task_embedding_map = np.load(os.path.join("/mnt/petrelfs/yangjiange/ATM/libero/task_embedding_caches/", "task_emb_bert.npy"),allow_pickle=True).item()
    ...
    def env_func(env_idx):
        ...
        if kwargs.get("camera_depths", False):
            base_env = LiberoIntrinsicsWrapper(base_env, img_h, img_w, kwargs["cameras"])
        base_env.seed(seed)
        return base_env
