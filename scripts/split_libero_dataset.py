...
def split_pretrain_dataset(files, train_folder, val_folder, train_ratio, link_folder):
    num_files = len(files)
    ...
    for f in train_files:
        # Create relative symlinks in the custom link folder
        link_path = os.path.join(link_folder, os.path.basename(f))
        os.makedirs(os.path.dirname(link_path), exist_ok=True)
        os.system('ln -s {} {}'.format(os.path.relpath(f, link_folder), link_path))
...
def split_bc_train_dataset(root_dir, pretrain_train_folder, link_folder, num_trains=[10, 20, 40, 80]):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder', type=str, default="/mnt/petrelfs/hetong/haoyi/ATM/data/atm_libero")
    parser.add_argument('--train_ratio', type=float, default=0.9)
    parser.add_argument('--link_folder', type=str, default="/mnt/petrelfs/yangjiange/ATM/datas")
    ...
    for suite_name in os.listdir(args.folder):
        for task_name in os.listdir(os.path.join(args.folder, suite_name)):
            ...
            if True:
                split_pretrain_dataset(files, train_folder, val_folder, args.train_ratio, args.link_folder)

                pretrain_train_folder = train_folder
                split_bc_train_dataset(root_dir, pretrain_train_folder, args.link_folder, num_trains=[2, 5, 10, 20, 30, 40, 45])
...
#                     pretrain_train_folder = train_folder
#                     split_bc_train_dataset(root_dir, pretrain_train_folder, num_trains=[10])
