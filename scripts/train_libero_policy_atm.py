...
parser.add_argument(
    "-tt",
    "--track-transformer",
    default="/mnt/petrelfs/yangjiange/ATM/results/track_transformer/0913_libero_track_transformer_libero-goal_ep301_1618--cvpr",
    help="Then path to the trained track transformer.",
)
...
train_path_list = [
    f"{root_dir}/{suite_name}/{task_dir}/bc_train_{NUM_DEMOS}"
    #f"{root_dir}/{suite_name}/{task_dir}"
    for task_dir in task_dir_list
]

val_path_list = [
    f"{root_dir}/{suite_name}/{task_dir}/val" for task_dir in task_dir_list
]
...
for seed in range(1):
    commond = (
        f'python -m engine.train_bc --config-name={CONFIG_NAME} train_gpus="{train_gpu_ids}" '
