...
def get_task_name_from_file_name(file_name):
    print("get_task_name_from_file_name", file_name)
...
def get_task_embs(cfg, descriptions):
    ...
    if cfg.task_embedding_format == "bert":
        ...
        print("cfg.data.max_word_len", cfg.data.max_word_len)
        tokens = tz(
...
def get_task_bert_embs(libero_root_dir):
    libero_h5_files = glob(os.path.join(libero_root_dir, "*/*_demo.hdf5"))
    print(libero_h5_files)
    task_names = set(
        [
            get_task_name_from_file_name(os.path.basename(file).split(".")[0])
            for file in libero_h5_files
        ]
    )
    ...
    if not os.path.exists("libero/task_embedding_caches/task_emb_bert.npy"):
        ...
        task_name_to_emb = np.load(
            "libero/task_embedding_caches/task_emb_bert.npy", allow_pickle=True
        ).item()
...
def track_and_remove(tracker, video, points, var_threshold=10.0):
    ...
    pred_tracks, pred_vis = tracker(
        video, queries=points, backward_tracking=True
    )  # [1, T, N, 2]
...
def collect_states_from_demo(
    ...
    root_grp = (
        h5_file.create_group("root") if "root" not in h5_file else h5_file["root"]
    )
    ...
    if "extra_states" not in root_grp:
        ...
        for state_key in EXTRA_STATES_KEYS:
            extra_states_grp.create_dataset(
                state_key, data=np.array(demos_group[demo_k]["obs"][state_key])
            )
    ...
    for view in view_names:
        ...
        rgb = rgb[
            :, ::-1, :, :
        ].copy()  # The images in the raw Libero dataset is upsidedown, so we need to flip it
        rgb = rearrange(rgb, "t h w c -> t c h w")
        T, C, H, W = rgb.shape

        pred_tracks, pred_vis = track_through_video(
            rgb, track_model, num_points=num_points
        )
        ...
        view_grp = (
            root_grp.create_group(view) if view not in root_grp else root_grp[view]
        )
...
def get_attrs_and_view_names(demo_h5):
    ...
    views = [
        name.replace("robot0_", "") if name.endswith("eye_in_hand") else name
        for name in views
    ]
    return attrs, views
...
def main(root, save, suite, skip_exist):
    ...
    for source_h5 in os.listdir(suite_dir):
        if "_demo.hdf5" not in source_h5:
            continue
